...
These files are uploaded to the **WordPress.org SVN `/assets/` directory**, which is a separate path from the plugin/theme `/trunk/` and is **not** included in the user-facing zip. They drive the listing on wordpress.org/plugins/delopay and wordpress.org/themes/delopay-shop.
...
svn co https://plugins.svn.wordpress.org/delopay/ delopay-svn
cp wporg-assets/plugin/* delopay-svn/assets/
cd delopay-svn && svn add assets/* && svn ci -m "Add directory listing assets"
```
