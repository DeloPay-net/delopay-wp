...
function delopay_shop_brand_name() {
	if ( class_exists( 'Delopay_Settings' ) ) {
		$name = Delopay_Settings::get( 'business_name' );
		if ( ! empty( $name ) ) {
			return $name;
		}
	}
...
function delopay_shop_plugin_configured() {
	...
	$profile_id = (string) Delopay_Settings::get( 'profile_id' );
...
function delopay_shop_footer_copy() {
	...
	if ( class_exists( 'Delopay_Settings' ) ) {
		$email   = Delopay_Settings::get( 'business_email' );
		$support = Delopay_Settings::get( 'business_support' );
		if ( $email ) {
