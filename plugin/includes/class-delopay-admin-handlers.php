...
class Delopay_Admin_Handlers {
	...
	public function save_product() {
		...
		check_admin_referer( 'delopay_save_product', 'delopay_product_nonce' );

		$id     = isset( $_POST['product_id'] ) ? (int) $_POST['product_id'] : 0;
		$action = $id > 0 ? 'edit' : 'new';

		$clean = Delopay_Products::sanitize_input( $_POST, $id );
		if ( is_wp_error( $clean ) ) {
			$this->redirect_form_error( 'product', $clean->get_error_message(), $action, $id );
		}

		$result = $id > 0
			? Delopay_Products::update( $id, $clean )
			: Delopay_Products::create( $clean );
		...
		Delopay_Admin_UI::redirect(
			array(
				'page'    => Delopay_Admin::SLUG_PRODUCTS,
				'action'  => 'edit',
	...
	public function delete_product() {
		...
		check_admin_referer( 'delopay_delete_product_' . $id );

		Delopay_Products::delete( $id );
	...
	public function export_products() {
		Delopay_Admin_UI::require_cap();
		check_admin_referer( 'delopay_export_products' );

		$payload = array(
			'version'     => 2,
			'exported_at' => gmdate( 'c' ),
			'site_url'    => home_url( '/' ),
			'categories'  => array_map( array( $this, 'shape_category_for_export' ), Delopay_Categories::list_all() ),
			'products'    => array_map( array( $this, 'shape_product_for_export' ), Delopay_Products::list_all( array( 'limit' => 10000 ) ) ),
		);
	...
	public function import_products() {
		Delopay_Admin_UI::require_cap();
		check_admin_referer( 'delopay_import_products', 'delopay_import_nonce' );
		...
		if ( ! is_array( $data ) ) {
			$this->redirect_import_error( __( 'The file is not valid JSON.', 'delopay' ) );
		}
		...
		if ( ! is_array( $items ) || empty( $items ) ) {
			$this->redirect_import_error( __( 'No products found in the file.', 'delopay' ) );
		}
		...
		$msg = ! empty( $cats )
			? sprintf(
				...
				__( 'Import complete: %1$d products created, %2$d skipped (existing SKU), %3$d failed; %4$d categories created, %5$d updated, %6$d failed.', 'delopay' ),
				$prod_stats['created'],
				$prod_stats['skipped'],
				$prod_stats['failed'],
				$cat_stats['created'],
			...
			: sprintf(
				...
				__( 'Import complete: %1$d created, %2$d skipped (existing SKU), %3$d failed.', 'delopay' ),
				$prod_stats['created'],
				$prod_stats['skipped'],
				$prod_stats['failed']
			);
	...
	private function read_uploaded_import_file() {
		if ( empty( $_FILES['import_file'] ) || UPLOAD_ERR_OK !== (int) $_FILES['import_file']['error'] ) {
			return new WP_Error( 'no_file', __( 'No file uploaded or upload failed.', 'delopay' ) );
		}
		...
		if ( $size <= 0 || $size > self::IMPORT_MAX_BYTES ) {
			return new WP_Error(
				...
				/* translators: %s: max file size like "5 MB" */
					__( 'File is too large or empty (max %s).', 'delopay' ),
		...
		if ( ! $ext_ok && ! $ext_fallback ) {
			return new WP_Error( 'bad_ext', __( 'File must be a .json export.', 'delopay' ) );
		}
		...
		if ( false === $contents || '' === $contents ) {
			return new WP_Error( 'empty', __( 'The uploaded file is empty.', 'delopay' ) );
		}
	...
	private function import_categories( array $cats ) {
		...
		foreach ( $cats as $cat_item ) {
			...
			$existing_before = Delopay_Categories::find( (string) ( $cat_item['slug'] ?? '' ), false );
			$result          = Delopay_Categories::upsert_by_slug( $cat_item );
	...
	private function import_product_rows( array $items ) {
		...
		foreach ( $items as $item ) {
			...
			$clean = Delopay_Products::sanitize_input( $item, 0 );
			if ( is_wp_error( $clean ) ) {
				++$stats[ 'sku_taken' === $clean->get_error_code() ? 'skipped' : 'failed' ];
				continue;
			}
			$result = Delopay_Products::create( $clean );
	...
	public function save_category() {
		Delopay_Admin_UI::require_cap();
		check_admin_referer( 'delopay_save_category', 'delopay_category_nonce' );

		$id     = isset( $_POST['category_id'] ) ? (int) $_POST['category_id'] : 0;
		$action = $id > 0 ? 'edit' : 'new';

		$clean = Delopay_Categories::sanitize_input( $_POST, $id );
		if ( is_wp_error( $clean ) ) {
			$this->redirect_form_error( 'category', $clean->get_error_message(), $action, $id );
		}

		$result = $id > 0
			? Delopay_Categories::update( $id, $clean )
			: Delopay_Categories::create( $clean );

		if ( is_wp_error( $result ) ) {
			$this->redirect_form_error( 'category', $result->get_error_message(), $action, $id );
		}

		Delopay_Plugin::ensure_category_page( $result );
		do_action( 'delopay_category_saved', $result, $id > 0 ? 'updated' : 'created' );

		Delopay_Admin_UI::redirect(
			array(
				'page'     => Delopay_Admin::SLUG_CATEGORIES,
				'action'   => 'edit',
	...
	public function delete_category() {
		...
		check_admin_referer( 'delopay_delete_category_' . $id );

		$result = Delopay_Categories::delete( $id, true );
		if ( is_wp_error( $result ) ) {
			Delopay_Admin_UI::redirect(
				array(
					'page'  => Delopay_Admin::SLUG_CATEGORIES,
