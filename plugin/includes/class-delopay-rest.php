...
class Delopay_REST {
	...
	public function require_first_party( WP_REST_Request $request ) {
		...
		if ( ! is_array( $home ) || empty( $home['host'] ) ) {
			return $this->forbidden( __( 'Origin check failed.', 'delopay' ) );
		}
		...
		return $this->forbidden( __( 'This endpoint requires a same-origin request with a valid REST nonce.', 'delopay' ) );
	}
	...
	public function list_products( WP_REST_Request $request ) {
		...
		$products = Delopay_Products::list_published( self::PUBLIC_LIST_LIMIT, $category );
	...
	public function create_order( WP_REST_Request $request ) {
		...
		$order_id    = Delopay_Orders::new_order_id();
		$safe_return = $this->safe_return_url( $request->get_param( 'return_url' ) );
		$return_url  = $safe_return ? $safe_return : Delopay_Settings::get_complete_url();
		$return_url  = add_query_arg( 'order_id', $order_id, $return_url );

		$placeholder = Delopay_Orders::create_pending(
			array(
				'order_id'     => $order_id,
		...
		if ( ! $placeholder ) {
			Delopay_Log::error( 'failed to insert local order placeholder', array( 'order_id' => $order_id ) );
			return self::error_response( __( 'failed to create local order', 'delopay' ), 500 );
		}

		$payment = $this->create_remote_payment( $order_id, $validated, $return_url );
		if ( $payment instanceof WP_REST_Response ) {
			Delopay_Orders::delete_pending( $order_id );
			return $payment;
		}

		$order = Delopay_Orders::attach_payment(
			array(
				'order_id'    => $order_id,
		...
		if ( ! $order ) {
			Delopay_Log::error( 'attach_payment returned null', array( 'order_id' => $order_id ) );
			return self::error_response( __( 'failed to finalize local order', 'delopay' ), 500 );
		}
	...
	private function normalize_items_input( WP_REST_Request $request ) {
		...
		if ( ! $ref ) {
			return self::error_response( __( 'product_id (or sku) required', 'delopay' ), 400 );
		}
	...
	private function validate_lines( array $items_input ) {
		if ( count( $items_input ) > self::MAX_LINES_PER_ORDER ) {
			return self::error_response(
				...
				sprintf( __( 'too many line items (max %d)', 'delopay' ), self::MAX_LINES_PER_ORDER ),
				422
			);
		}
		...
		if ( $amount_minor <= 0 ) {
			return self::error_response( __( 'order amount must be greater than zero', 'delopay' ), 400 );
		}
		if ( $amount_minor > self::MAX_AMOUNT_MINOR ) {
			return self::error_response( __( 'order amount exceeds the per-order maximum', 'delopay' ), 422 );
		}
	...
	private function validate_one_line( $it, $current_currency ) {
		...
		if ( ! $ref || $qty < 1 ) {
			return self::error_response( __( 'invalid line item', 'delopay' ), 400 );
		}
		if ( $qty > self::MAX_QUANTITY_PER_LINE ) {
			return self::error_response(
				/* translators: %d: maximum allowed quantity per line item. */
				sprintf( __( 'quantity exceeds max of %d', 'delopay' ), self::MAX_QUANTITY_PER_LINE ),
				422
			);
		}
		...
		if ( ! $product ) {
			return self::error_response(
				...
				sprintf( __( 'unknown product: %s', 'delopay' ), (string) $ref ),
				400
			);
		}

		if ( null !== $current_currency && strtoupper( (string) $current_currency ) !== strtoupper( (string) $product['currency'] ) ) {
			return self::error_response( __( 'cannot mix currencies in a single order', 'delopay' ), 400 );
		}
	...
	private function create_remote_payment( $order_id, array $validated, $return_url ) {
		...
		if ( is_wp_error( $payment ) ) {
			...
			return self::error_response(
				self::admin_error_message(
					__( 'Could not start payment. Please try again.', 'delopay' ),
		...
		if ( empty( $payment['payment_id'] ) || empty( $payment['merchant_id'] ) ) {
			...
			return self::error_response( __( 'DeloPay response missing payment_id/merchant_id', 'delopay' ), 502 );
		}
	...
	private static function admin_error_message( $public_message, $detail ) {
		...
		return sprintf( __( '%1$s (admin: %2$s)', 'delopay' ), $public_message, $detail );
	}
	...
	private function build_create_payment_params( $order_id, array $validated, $return_url ) {
		...
		return array(
			...
			'currency'                    => $validated['currency'],
			'confirm'                     => false,
			'capture_method'              => 'manual' === Delopay_Settings::get( 'capture_method' ) ? 'manual' : 'automatic',
			'return_url'                  => $return_url,
			'description'                 => sprintf(
				/* translators: 1: order id, 2: site name */
				__( 'Order %1$s on %2$s', 'delopay' ),
				$order_id,
				get_bloginfo( 'name' )
			),
			'payment_link'                => true,
	...
	public function get_order( WP_REST_Request $request ) {
		$id    = $request['id'];
		$order = Delopay_Orders::find( $id );
		if ( ! $order ) {
			return self::error_response( __( 'order not found', 'delopay' ), 404 );
		}
	...
	private function shape_order_status( $order ) {
		return array(
			...
			'refunded_minor'  => Delopay_Orders::refunded_total( $order['order_id'] ),
		);
	}

	private function maybe_refresh_order_status( $order ) {
		if ( in_array( $order['status'], Delopay_Orders::terminal_statuses(), true ) ) {
			return $order;
		}
		...
		$updated = Delopay_Orders::update_status(
			$order['payment_id'],
			$remote['status'],
	...
	public function admin_refund( WP_REST_Request $request ) {
		...
		if ( null !== $requested && ( ! is_numeric( $requested ) || $requested <= 0 ) ) {
			return self::error_response( __( 'amount_minor must be a positive number', 'delopay' ), 400 );
		}

		if ( ! Delopay_Orders::acquire_refund_lock( $order['order_id'] ) ) {
			return self::error_response(
				__( 'another refund is in progress for this order; try again in a moment', 'delopay' ),
				409
			);
		}
		...
		try {
			...
			Delopay_Orders::release_refund_lock( $order['order_id'] );
		}
	}
	...
	public function admin_capture( WP_REST_Request $request ) {
		$order = Delopay_Orders::find( (string) $request->get_param( 'order_id' ) );
		if ( ! $order ) {
			return self::error_response( __( 'order not found', 'delopay' ), 404 );
		}
		...
		Delopay_Orders::update_status( $order['payment_id'], $status, null, null, false, (string) $order['order_id'] );
	...
	private function process_refund( $order, $requested, $reason_input ) {
		$refunded_so_far = Delopay_Orders::refunded_total( $order['order_id'] );
		$remaining       = (int) $order['amount_minor'] - $refunded_so_far;
		if ( $remaining <= 0 ) {
			return self::error_response( __( 'order already fully refunded', 'delopay' ), 400 );
		}
		...
		if ( $amount > $remaining ) {
			return self::error_response(
				...
				sprintf( __( 'refund exceeds remaining %d', 'delopay' ), $remaining ),
				400
			);
		}
		...
		if ( '' === $refund_id ) {
			Delopay_Log::error( 'DeloPay refund response missing refund_id', array( 'order_id' => $order['order_id'] ) );
			return new WP_REST_Response(
				array(
					'error'  => __( 'DeloPay accepted the refund but returned no refund_id; not recorded locally. Reconcile manually from the DeloPay dashboard.', 'delopay' ),
					'detail' => $refund,
				),
				502
			);
	...
	private function detect_over_refund( $order, $booked_amount, $refund_id, $refund_payload ) {
		$projected_total = Delopay_Orders::refunded_total( $order['order_id'] ) + $booked_amount;
		...
		return new WP_REST_Response(
			array(
				'error'  => __( 'refund created at DeloPay but would over-refund locally; reconcile manually', 'delopay' ),
	...
	private function build_checkout_url( $order ) {
		$base = rtrim( (string) Delopay_Settings::get( 'checkout_base_url' ), '/' );
		if ( '' === $base ) {
			return '';
		}
