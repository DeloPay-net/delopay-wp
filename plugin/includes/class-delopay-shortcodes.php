...
class Delopay_Shortcodes {
	...
	private function ensure_assets() {
		wp_enqueue_style( 'delopay-frontend' );
		wp_enqueue_script( 'delopay-frontend' );
	}
	...
	private static function empty_state( $message ) {
		return '<p class="delopay-empty">' . esc_html( $message ) . '</p>';
	}
	...
	public function products( $atts ) {
		...
		$products        = Delopay_Products::list_published( (int) $atts['limit'], $category_filter );

		if ( empty( $products ) ) {
			return self::empty_state( __( 'No products yet.', 'delopay' ) );
		}
	...
	public function categories( $atts ) {
		...
		$categories = Delopay_Categories::list_published();
		if ( empty( $categories ) ) {
			return self::empty_state( __( 'No categories yet.', 'delopay' ) );
		}
	...
	private function render_categories_grid( array $categories, array $counts, $columns ) {
		?>
		<div class="delopay-grid delopay-grid-categories" style="--cols: <?php echo (int) $columns; ?>;">
			<?php
			foreach ( $categories as $cat ) :
				$count   = isset( $counts[ (int) $cat['id'] ] ) ? (int) $counts[ (int) $cat['id'] ] : 0;
				$cat_url = Delopay_Categories::page_url_for_slug( $cat['slug'] );
				$url     = $cat_url ? $cat_url : home_url( '/' );
				?>
				<article class="delopay-category" data-category-slug="<?php echo esc_attr( $cat['slug'] ); ?>">
					<a class="delopay-category-link" href="<?php echo esc_url( $url ); ?>">
						<div class="delopay-category-body">
							<h2 class="delopay-category-name"><?php echo esc_html( $cat['name'] ); ?></h2>
							<?php if ( $cat['excerpt'] ) : ?>
								<p class="delopay-category-excerpt"><?php echo esc_html( $cat['excerpt'] ); ?></p>
							<?php endif; ?>
							<p class="delopay-category-count">
								<?php
								/* translators: %d: number of products in the category */
								echo esc_html( sprintf( _n( '%d product', '%d products', $count, 'delopay' ), $count ) );
								?>
							</p>
						</div>
					</a>
	...
	public function category_hero( $atts ) {
		...
		$category = '' !== $slug ? Delopay_Categories::find( $slug, false ) : null;
	...
	private function render_hero( $category, $active ) {
		?>
		<section class="ds-hero <?php echo esc_attr( $active ? 'delopay-hero' : 'ds-hero-empty delopay-hero-empty' ); ?>"
			<?php
			if ( ! $active ) :
				?>
	...
	public function product( $atts ) {
		...
		if ( ! $ref ) {
			return self::empty_state( __( 'Missing product id or sku.', 'delopay' ) );
		}

		$product = Delopay_Products::find( $ref );
		if ( ! $product ) {
	...
	private function render_cart_widget( $shop_url, $checkout_url ) {
		...
		<header class="ds-page-head">
			<h1><?php esc_html_e( 'Your cart', 'delopay' ); ?></h1>
		</header>
		<div class="delopay-cart"
			data-checkout-url="<?php echo esc_attr( $checkout_url ? $checkout_url : '' ); ?>"
			data-shop-url="<?php echo esc_attr( $shop_url ); ?>">
			<div class="delopay-cart-loading"><?php esc_html_e( 'Loading your cart…', 'delopay' ); ?></div>

			<div class="delopay-cart-empty" hidden>
				<p><?php esc_html_e( 'Your cart is empty.', 'delopay' ); ?></p>
				<a class="delopay-buy-button" href="<?php echo esc_url( $shop_url ); ?>"><?php esc_html_e( 'Browse products', 'delopay' ); ?></a>
			</div>

			<div class="delopay-cart-content" hidden>
				<ul class="delopay-cart-items"></ul>

				<div class="delopay-cart-summary">
					<dl>
						<dt><?php esc_html_e( 'Subtotal', 'delopay' ); ?></dt>
						<dd data-field="subtotal">—</dd>
					</dl>
					<?php
					$cart_mode      = Delopay_Settings::cart_checkout_mode();
					$show_embedded  = in_array( $cart_mode, array( 'both', 'embedded' ), true );
					$show_external  = in_array( $cart_mode, array( 'both', 'external' ), true );
					$external_class = 'delopay-buy-button delopay-cart-checkout-external';
					if ( $show_embedded && $show_external ) {
						$external_class .= ' delopay-buy-button-secondary';
					}
					?>
					<div class="delopay-cart-checkout-row">
						<?php if ( $show_embedded ) : ?>
							<button type="button" class="delopay-buy-button delopay-cart-checkout" disabled>
								<?php esc_html_e( 'Proceed to checkout', 'delopay' ); ?>
							</button>
						<?php endif; ?>
						<?php if ( $show_external ) : ?>
							<button type="button" class="<?php echo esc_attr( $external_class ); ?>" disabled>
								<?php echo esc_html( $show_embedded ? __( 'Proceed to external checkout', 'delopay' ) : __( 'Proceed to checkout', 'delopay' ) ); ?>
							</button>
						<?php endif; ?>
					</div>
					<p class="delopay-cart-error" hidden></p>
				</div>
			</div>
	...
	private function render_cart_row_template() {
		?>
		<template class="delopay-cart-row-template">
			<li class="delopay-cart-row" data-product-id="">
				<div class="delopay-cart-thumb"></div>
				<div class="delopay-cart-meta">
					<h3 data-field="name"></h3>
					<p data-field="unit_price" class="delopay-cart-unit"></p>
				</div>
				<div class="delopay-cart-qty">
					<button type="button" class="delopay-cart-dec" aria-label="<?php esc_attr_e( 'Decrease', 'delopay' ); ?>">−</button>
					<span data-field="quantity">1</span>
					<button type="button" class="delopay-cart-inc" aria-label="<?php esc_attr_e( 'Increase', 'delopay' ); ?>">+</button>
				</div>
				<div class="delopay-cart-line-total" data-field="line_total"></div>
				<button type="button" class="delopay-cart-remove" aria-label="<?php esc_attr_e( 'Remove', 'delopay' ); ?>">×</button>
			</li>
		</template>
		<?php
	}
	...
	public function checkout( $atts ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.Found -- WP shortcode callback signature.
		...
		$product = $ref ? Delopay_Products::find( $ref ) : null;
	...
	private function render_checkout_widget( $product, $quantity ) {
		?>
		<header class="ds-page-head">
			<h1><?php esc_html_e( 'Checkout', 'delopay' ); ?></h1>
			<p class="ds-sub"><?php esc_html_e( 'Review your order and pay with DeloPay.', 'delopay' ); ?></p>
		</header>
		<div class="delopay-checkout"
			<?php if ( $product ) : ?>
			...
			<aside class="delopay-checkout-pay">
				<div class="delopay-checkout-status">
					<?php esc_html_e( 'Preparing secure payment…', 'delopay' ); ?>
				</div>
				<div class="delopay-checkout-iframe-wrap" hidden>
					<iframe
						class="delopay-checkout-iframe"
						allow="payment *"
						referrerpolicy="origin"
					></iframe>
				</div>
				<div class="delopay-checkout-error" hidden></div>
			</aside>
		</div>
		<?php
	}
	...
	private function render_checkout_product_summary( $product, $quantity ) {
		...
		?>
		<section class="delopay-checkout-summary">
			<h2><?php esc_html_e( 'Order summary', 'delopay' ); ?></h2>
			<ul class="delopay-checkout-lines">
				<li>
			...
			<div class="delopay-checkout-total">
				<span><?php esc_html_e( 'Total', 'delopay' ); ?></span>
	...
	private function render_checkout_cart_summary() {
		?>
		<section class="delopay-checkout-summary" data-cart-summary hidden>
			<h2><?php esc_html_e( 'Order summary', 'delopay' ); ?></h2>
			<ul class="delopay-checkout-lines"></ul>
			<div class="delopay-checkout-total">
				<span><?php esc_html_e( 'Total', 'delopay' ); ?></span>
	...
	private function render_complete_widget( $order_id ) {
		?>
		<div class="delopay-complete" data-order-id="<?php echo esc_attr( $order_id ); ?>">
			<?php if ( '' === $order_id ) : ?>
				<div class="delopay-complete-status is-failure">
					<?php esc_html_e( 'No order id in URL.', 'delopay' ); ?>
				</div>
			<?php else : ?>
				<div class="delopay-complete-icon" aria-hidden="true"></div>
				<div class="delopay-complete-status">
					<?php esc_html_e( 'Checking payment status…', 'delopay' ); ?>
				</div>
				<p class="delopay-complete-amount" data-field="amount" hidden></p>
				<p class="delopay-complete-message" data-field="message" hidden></p>
				<dl class="delopay-complete-details" hidden>
					<dt><?php esc_html_e( 'Order', 'delopay' ); ?></dt>
					<dd><code data-field="order_id"></code></dd>
					<dt><?php esc_html_e( 'Payment', 'delopay' ); ?></dt>
					<dd><code data-field="payment_id"></code></dd>
					<dt><?php esc_html_e( 'Status', 'delopay' ); ?></dt>
					<dd data-field="status"></dd>
				</dl>
				<div class="delopay-complete-actions">
					<a class="delopay-complete-link" href="<?php echo esc_url( home_url( '/' ) ); ?>">
						<?php esc_html_e( 'Back to shop', 'delopay' ); ?>
					</a>
				</div>
			<?php endif; ?>
		</div>
			...
			<article class="delopay-product" data-product-id="<?php echo esc_attr( $product['id'] ); ?>">
				...
				<div class="delopay-product-body">
					...
					<div class="delopay-product-row">
						...
						<button
							type="button"
							class="delopay-buy-button delopay-add-to-cart"
							data-product-id="<?php echo esc_attr( $product['id'] ); ?>"
							data-product-name="<?php echo esc_attr( $product['name'] ); ?>">
							<?php esc_html_e( 'Add to cart', 'delopay' ); ?>
						</button>
					</div>
				</div>
			</article>
