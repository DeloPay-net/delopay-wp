...
class Delopay_Products {
	...
	public static function find( $product_ref, $only_active = true ) {
		global $wpdb;
		$table = Delopay_Orders::table_products();
	...
	private static function hydrate_many( array $rows ) {
		...
		$category_map = Delopay_Categories::find_many( $category_ids );
		$out          = array();
		foreach ( $rows as $row ) {
	...
	private static function resolve_category_filter( $category ) {
		...
		$cat = Delopay_Categories::find( (string) $category, false );
		if ( ! $cat ) {
			return false;
		}
		return array(
	...
	public static function sanitize_input( $input, $existing_id = 0 ) {
		...
		if ( '' === $name ) {
			return new WP_Error( 'name_required', __( 'Name is required.', 'delopay' ) );
		}
		...
		if ( '' !== $sku && self::sku_in_use( $sku, $existing_id ) ) {
			...
			return new WP_Error( 'sku_taken', sprintf( __( 'SKU "%s" is already used by another product.', 'delopay' ), $sku ) );
		}
	...
	private static function clean_price( $input ) {
		...
		if ( $price_decimal < 0 || ! is_finite( $price_decimal ) ) {
			return new WP_Error( 'price_invalid', __( 'Price must be a non-negative number.', 'delopay' ) );
		}
	...
	private static function clean_currency( $input ) {
		$currency = isset( $input['currency'] )
			...
			: '';
		if ( '' === $currency ) {
			$currency = strtoupper( (string) Delopay_Settings::get( 'currency' ) );
		}
		if ( ! preg_match( '/^[A-Z]{3}$/', $currency ) ) {
			return new WP_Error( 'currency_invalid', __( 'Currency must be a 3-letter ISO 4217 code.', 'delopay' ) );
		}
		return $currency;
	}
	...
	private static function clean_image( $input ) {
		...
		if ( '' !== $image_url_raw ) {
			...
			if ( '' === $image_url ) {
				return new WP_Error( 'image_url_invalid', __( 'Image URL must be a valid http(s) URL.', 'delopay' ) );
			}
		}
		if ( $image_id > 0 ) {
			$image_url = '';
	...
	private static function resolve_category_input( $input ) {
		...
		if ( '' !== $ref ) {
			$cat = is_numeric( $ref )
				? Delopay_Categories::find_for_admin( (int) $ref )
				: Delopay_Categories::find( $ref, false );
			if ( $cat ) {
				return (int) $cat['id'];
			}
		}

		$home = Delopay_Categories::find( 'home', false );
	...
	public static function create( $clean ) {
		...
		if ( false === $wpdb->insert( Delopay_Orders::table_products(), $row ) ) {
			return new WP_Error( 'db_error', __( 'Could not create product.', 'delopay' ) );
		}
	...
	public static function update( $id, $clean ) {
		...
		if ( ! self::find_for_admin( $id ) ) {
			return new WP_Error( 'not_found', __( 'Product not found.', 'delopay' ) );
		}
		$row               = $clean;
		$row['updated_at'] = current_time( 'mysql', true );

		if ( false === $wpdb->update( Delopay_Orders::table_products(), $row, array( 'id' => $id ) ) ) {
			return new WP_Error( 'db_error', __( 'Could not update product.', 'delopay' ) );
		}
	...
	public static function delete( $id ) {
		global $wpdb;
		return false !== $wpdb->delete( Delopay_Orders::table_products(), array( 'id' => (int) $id ) );
	}
