...
class Delopay_Connect {

	const TRANSIENT_PREFIX = 'delopay_connect_';
	const STATE_TTL        = 600;
	const STATE_REGEX      = '/^[A-Za-z0-9_\-]{16,128}$/';

	const OPTION_MERCHANT_ID  = 'delopay_connect_merchant_id';
	const OPTION_API_KEY_ID   = 'delopay_connect_api_key_id';
	const OPTION_CONNECTED_AT = 'delopay_connect_connected_at';
	...
	public function start( WP_REST_Request $request ) {
		...
		if ( '' === $control_center_url ) {
			return new WP_REST_Response( array( 'error' => __( 'Control center URL is not configured.', 'delopay' ) ), 400 );
		}
		...
		$return_url  = Delopay_Settings::get_complete_url();
	...
	private function resolve_environment( $param ) {
		...
		$valid = array( Delopay_Settings::ENV_PRODUCTION, Delopay_Settings::ENV_SANDBOX, Delopay_Settings::ENV_CUSTOM );
		return in_array( $env, $valid, true ) ? $env : Delopay_Settings::get_environment();
	}
	...
	private function resolve_control_center_url( $env, $param ) {
		...
		if ( Delopay_Settings::ENV_CUSTOM === $env ) {
			...
			if ( '' === $url ) {
				$url = (string) Delopay_Settings::get( 'control_center_url' );
			}
		} else {
	...
	public function complete( WP_REST_Request $request ) {
		...
		if ( '' === $state || ! preg_match( self::STATE_REGEX, $state ) ) {
			return self::render_complete_page( false, __( 'Missing or invalid connect state.', 'delopay' ) );
		}
		...
		if ( ! is_array( $pending ) ) {
			return self::render_complete_page( false, __( 'Connect link expired or already used. Please try again.', 'delopay' ) );
		}

		$creds = $this->extract_credentials( $body );
		if ( '' === $creds['api_key'] || '' === $creds['webhook_secret'] || '' === $creds['profile_id'] ) {
			return self::render_complete_page( false, __( 'Control center returned incomplete credentials.', 'delopay' ) );
		}

		$probe_base = rtrim( (string) $pending['control_center_url'], '/' ) . '/api';
		$probe      = ( new Delopay_Client( $creds['api_key'], $probe_base, $creds['profile_id'] ) )->probe_credentials();
		if ( empty( $probe['ok'] ) ) {
			Delopay_Log::warning(
			...
			return self::render_complete_page(
				false,
				__( 'DeloPay rejected the credentials returned by the control center. Please retry the Connect flow.', 'delopay' )
			);
		}
	...
	private function persist_credentials( array $creds, array $pending ) {
		...
		$env_urls = Delopay_Settings::env_urls();
		if ( Delopay_Settings::ENV_CUSTOM !== $pending['environment'] && isset( $env_urls[ $pending['environment'] ] ) ) {
	...
	private function revoke_remote_key() {
		...
		$base = rtrim( (string) Delopay_Settings::get_api_base_url(), '/' );
		if ( '' === $base ) {
			return array( 'skipped', null );
		}

		$response = wp_remote_request(
			$base . '/api-keys/' . rawurlencode( $merchant_id ) . '/' . rawurlencode( $key_id ),
			array(
				'method'  => 'DELETE',
				...
				'headers' => array(
					...
					'User-Agent' => 'delopay/' . DELOPAY_VERSION,
				),
			)
		);
	...
	private static function render_complete_page( $ok, $error_message = '', $opener_origin = '' ) {
		...
		$title  = $ok
			...
			: __( 'Connection failed', 'delopay' );
		$body   = $ok
			? __( 'You can close this window — your site is now connected to DeloPay.', 'delopay' )
			: $error_message;
		...
		$script =
			'(function(){try{if(window.opener){window.opener.postMessage(' . $payload . ',' . wp_json_encode( $post_message_origin ) . ');}}catch(e){}'
			. 'setTimeout(function(){try{window.close();}catch(e){}},800);})();';

		// This page is a self-contained popup document (no wp_head/wp_footer), so the
		// registered assets are emitted with wp_print_styles()/wp_print_scripts() below.
		wp_register_style( 'delopay-connect-complete', false, array(), DELOPAY_VERSION );
		wp_add_inline_style( 'delopay-connect-complete', $styles );
		wp_register_script( 'delopay-connect-complete', false, array(), DELOPAY_VERSION, true );
		wp_add_inline_script( 'delopay-connect-complete', $script );

		ob_start();
		wp_print_styles( 'delopay-connect-complete' );
		$style_tag = (string) ob_get_clean();

		ob_start();
		wp_print_scripts( 'delopay-connect-complete' );
		$script_tag = (string) ob_get_clean();
