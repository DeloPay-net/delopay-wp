 ...
 */
class WP_Delopay_Admin_Page_Disputes extends WP_Delopay_Admin_Page {

	public function slug() {
		return WP_Delopay_Admin::SLUG_DISPUTES;
	}

	public function label() {
		return __( 'Disputes', 'wp-delopay' );
	}
	...
	public function render() {
		...
		<div class="wrap wp-delopay-wrap">
			<h1><?php esc_html_e( 'Disputes', 'wp-delopay' ); ?></h1>
			<?php
			if ( ! $client->is_ready() ) {
				echo '<p>' . esc_html__( 'Connect to DeloPay in Settings to view disputes.', 'wp-delopay' ) . '</p></div>';
				return;
			}
	...
	private function render_list( WP_Delopay_Client $client ) {
		...
		if ( empty( $disputes ) ) {
			echo '<p>' . esc_html__( 'No disputes.', 'wp-delopay' ) . '</p>';
			return;
		}
		?>
		<table class="widefat striped">
			<thead>
				<tr>
					<th><?php esc_html_e( 'Dispute', 'wp-delopay' ); ?></th>
					<th><?php esc_html_e( 'Payment', 'wp-delopay' ); ?></th>
					<th><?php esc_html_e( 'Amount', 'wp-delopay' ); ?></th>
					<th><?php esc_html_e( 'Stage', 'wp-delopay' ); ?></th>
			...
			foreach ( $disputes as $d ) :
				...
				$detail_url = WP_Delopay_Admin_UI::page_url( WP_Delopay_Admin::SLUG_DISPUTES, array( 'dispute_id' => $dispute_id ) );
				?>
				<tr>
					...
					<td><?php WP_Delopay_Admin_UI::status_badge( isset( $d['dispute_status'] ) ? (string) $d['dispute_status'] : 'unknown' ); ?></td>
					<td><?php echo esc_html( isset( $d['created_at'] ) ? (string) $d['created_at'] : '—' ); ?></td>
					<td><a class="button" href="<?php echo esc_url( $detail_url ); ?>"><?php esc_html_e( 'Open', 'wp-delopay' ); ?></a></td>
				</tr>
			<?php endforeach; ?>
			</tbody>
		</table>
	...
	private function render_detail( WP_Delopay_Client $client, $dispute_id ) {
		$dispute  = $client->retrieve_dispute( $dispute_id );
		$list_url = WP_Delopay_Admin_UI::page_url( WP_Delopay_Admin::SLUG_DISPUTES );
		echo '<p><a href="' . esc_url( $list_url ) . '">&larr; ' . esc_html__( 'All disputes', 'wp-delopay' ) . '</a></p>';
	...
	private function format_amount( array $d ) {
		...
		$currency = isset( $d['currency'] ) ? (string) $d['currency'] : WP_Delopay_Settings::get( 'currency' );
		return WP_Delopay_Admin_UI::format_money( (int) $d['amount'], $currency );
	}
}
