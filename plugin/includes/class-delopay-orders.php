...
class Delopay_Orders {
	...
	public static function find( $id_or_payment ) {
		$row = self::get_row( self::table_orders(), 'order_id = %s OR payment_id = %s', $id_or_payment, $id_or_payment );
		return $row ? self::hydrate( $row ) : null;
	}
	...
	public static function record_refund( $data ) {
		...
		if ( empty( $data['refund_id'] ) || ! is_string( $data['refund_id'] ) ) {
			Delopay_Log::warning( 'refusing to record refund without refund_id' );
			return null;
		}
	...
	public static function reconcile_pending_refunds() {
		if ( ! class_exists( 'Delopay_Settings' ) || ! Delopay_Settings::is_configured() ) {
			return;
		}
	...
	private static function reconcile_refund_row( $row, $client ) {
		...
		if ( is_wp_error( $remote ) || ! is_array( $remote ) ) {
			Delopay_Log::warning( 'reconcile: failed to retrieve refund ' . $refund_id );
			return;
		}
