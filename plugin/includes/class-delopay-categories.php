...
class Delopay_Categories {
	...
	public static function find( $ref, $only_active = true ) {
		global $wpdb;
		$table = Delopay_Orders::table_categories();
	...
	public static function product_counts() {
		global $wpdb;
		$products = Delopay_Orders::table_products();
		$rows     = $wpdb->get_results(
	...
	public static function sanitize_input( $input, $existing_id = 0 ) {
		...
		if ( self::slug_in_use( $slug, $existing_id ) ) {
			...
			return new WP_Error( 'slug_taken', sprintf( __( 'Slug "%s" is already used by another category.', 'delopay' ), $slug ) );
		}
	...
	private static function clean_slug( $input, $name ) {
		...
		if ( '' === $slug ) {
			return new WP_Error( 'slug_invalid', __( 'Slug must contain at least one URL-safe character.', 'delopay' ) );
		}
		if ( strlen( $slug ) > self::MAX_SLUG_LENGTH ) {
			$slug = substr( $slug, 0, self::MAX_SLUG_LENGTH );
		}
		if ( in_array( $slug, self::RESERVED_SLUGS, true ) ) {
			return new WP_Error( 'slug_reserved', __( 'That slug is reserved.', 'delopay' ) );
		}
		return $slug;
	}
	...
	public static function create( $clean ) {
		...
		if ( false === $wpdb->insert( Delopay_Orders::table_categories(), $row ) ) {
			return new WP_Error( 'db_error', __( 'Could not create category.', 'delopay' ) );
		}
	...
	public static function update( $id, $clean ) {
		...
		if ( ! self::find_for_admin( $id ) ) {
			return new WP_Error( 'not_found', __( 'Category not found.', 'delopay' ) );
		}
		$row               = $clean;
		$row['updated_at'] = current_time( 'mysql', true );

		if ( false === $wpdb->update( Delopay_Orders::table_categories(), $row, array( 'id' => $id ) ) ) {
			return new WP_Error( 'db_error', __( 'Could not update category.', 'delopay' ) );
		}
	...
	public static function delete( $id, $force = false ) {
		...
		$wpdb->delete( Delopay_Orders::table_categories(), array( 'id' => $id ) );
		return true;
	}
