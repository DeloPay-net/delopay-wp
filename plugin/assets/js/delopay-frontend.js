...
(function () {
	...
	if (typeof Delopay === 'undefined') {
		return;
	}

	const I = Delopay.i18n || {};
	const CART_KEY = 'delopay_cart_v1';
	const CART_EVENT = 'delopay-cart-changed';
	...
	document.addEventListener('DOMContentLoaded', () => {
		...
		document.querySelectorAll('.delopay-add-to-cart').forEach(initAddToCart);
		document.querySelectorAll('.delopay-cart').forEach(initCartPage);
		document.querySelectorAll('.delopay-checkout').forEach(initCheckout);
		document.querySelectorAll('.delopay-complete').forEach(initComplete);
	});
	...
	function initCartPage(root) {
		const els = {
			loading:          root.querySelector('.delopay-cart-loading'),
			empty:            root.querySelector('.delopay-cart-empty'),
			content:          root.querySelector('.delopay-cart-content'),
			items:            root.querySelector('.delopay-cart-items'),
			subtotal:         root.querySelector('[data-field="subtotal"]'),
			tpl:              root.querySelector('.delopay-cart-row-template'),
			checkout:         root.querySelector('.delopay-cart-checkout'),
			checkoutExternal: root.querySelector('.delopay-cart-checkout-external'),
			error:            root.querySelector('.delopay-cart-error'),
		};
		...
		function buildRow(tpl, product, qty, lineTotal) {
			...
			if (product.image_url) {
				...
				img.alt = '';
				img.loading = 'lazy';
				node.querySelector('.delopay-cart-thumb').appendChild(img);
			}
			...
			node.querySelector('.delopay-cart-inc').addEventListener('click', () => Cart.setQty(id, qty + 1));
			node.querySelector('.delopay-cart-dec').addEventListener('click', () => Cart.setQty(id, qty - 1));
			node.querySelector('.delopay-cart-remove').addEventListener('click', () => Cart.remove(id));
			return node;
		}
			...
			els.checkoutExternal.addEventListener('click', () => {
				...
				fetchJson(restUrl('orders'), {
					...
					headers: {
						...
						'X-WP-Nonce':   Delopay.nonce,
					},
					body: JSON.stringify({
						items,
						return_url: Delopay.completeUrl || undefined,
					}),
				})
					.then((res) => {
						if (!res.ok) {
	...
	function initCheckout(root) {
		const statusEl  = root.querySelector('.delopay-checkout-status');
		const wrapEl    = root.querySelector('.delopay-checkout-iframe-wrap');
		const iframeEl  = root.querySelector('.delopay-checkout-iframe');
		const errorEl   = root.querySelector('.delopay-checkout-error');
		...
		body.return_url = Delopay.completeUrl || undefined;
	...
	function renderCartSummary(summaryEl, items) {
		...
		fetchProductsByIds(ids).then((products) => {
			...
			const ul      = summaryEl.querySelector('.delopay-checkout-lines');
	...
	function initComplete(root) {
		...
		const els = {
			statusEl:  root.querySelector('.delopay-complete-status'),
			detailsEl: root.querySelector('.delopay-complete-details'),
			amountEl:  root.querySelector('[data-field="amount"]'),
			messageEl: root.querySelector('[data-field="message"]'),
			iconEl:    root.querySelector('.delopay-complete-icon'),
		};

		function load() {
			return fetchJson(restUrl('orders/' + encodeURIComponent(orderId)), {
				headers: { 'X-WP-Nonce': Delopay.nonce },
			}).then((res) => {
				const data = res.body;
				if (data && data.error) {
