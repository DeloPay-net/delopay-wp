...
(function () {
	...
	if (typeof DelopayAdmin === 'undefined') {
		return;
	}

	const I = DelopayAdmin.i18n || {};
	...
	function restCall(path, options) {
		...
		opts.headers = Object.assign({
			...
			'X-WP-Nonce': DelopayAdmin.nonce,
		}, opts.headers);
		return fetch(DelopayAdmin.restUrl + path, opts)
	...
	function initColorSwatches() {
		document.querySelectorAll('.delopay-color-input').forEach((input) => {
			const swatch = input.parentElement
				? input.parentElement.querySelector('.delopay-color-swatch')
				: null;
			if (!swatch) return;
	...
	function initCopyButtons() {
		document.querySelectorAll('.delopay-copy-btn').forEach((btn) => {
			btn.addEventListener('click', () => copyFromTarget(btn));
		});
		document.querySelectorAll('.delopay-copy-row input[readonly]').forEach((input) => {
	...
	function copyFromTarget(btn) {
		...
		const restore = (label, ok) => {
			...
			btn.classList.toggle('delopay-copy-ok', !!ok);
			setTimeout(() => {
				btn.textContent = original;
				btn.classList.remove('delopay-copy-ok');
			}, COPY_FEEDBACK_MS);
		};
	...
	function initImagePickers() {
		document.querySelectorAll('.delopay-image-picker').forEach(initImagePicker);
	}

	function initImagePicker(root) {
		const pick     = root.querySelector('.delopay-image-pick');
		const clear    = root.querySelector('.delopay-image-clear');
		const idInput  = root.querySelector('.delopay-image-id');
		const urlInput = root.querySelector('.delopay-image-url');
		const preview  = root.querySelector('.delopay-image-preview');
		...
		function showPreview(url) {
			...
			img.addEventListener('error', () => {
				preview.innerHTML = '<div class="delopay-image-empty">' + (I.imageLoadFailed || 'Could not load image') + '</div>';
			});
			preview.appendChild(img);
		}

		function showEmpty() {
			preview.innerHTML = '<div class="delopay-image-empty">' + empty + '</div>';
		}
	...
	function initEnvironmentSelectors() {
		document.querySelectorAll('.delopay-env-select').forEach(initEnvironmentSelector);
	}

	function initEnvironmentSelector(select) {
		const customRows      = document.querySelectorAll('.delopay-env-custom-row');
		const summary         = document.querySelector('[data-env-summary]');
		const ccInput         = document.getElementById('delopay_control_center_url');
		const coInput         = document.getElementById('delopay_checkout_base_url');
		const projInput       = document.getElementById('delopay_project_id');
		const profInput       = document.getElementById('delopay_profile_id');
	...
	function initImportToggle() {
		document.querySelectorAll('.delopay-import-toggle').forEach((toggle) => {
	...
	function initDeleteConfirms() {
		document.querySelectorAll('.delopay-delete-product').forEach((link) => {
			...
			restCall('connect/start', {
				...
				.then((res) => {
					...
					messageHandler = (event) => {
						...
						if (!data || data.source !== 'delopay-connect') return;
	...
	function initCaptureControls() {
		document.querySelectorAll('.delopay-capture-controls').forEach((box) => {
			const orderId  = box.dataset.orderId;
			const statusEl = box.querySelector('.delopay-capture-status');
	...
	function initRefundForms() {
		document.querySelectorAll('.delopay-refund-form').forEach(initRefundForm);
	}
	...
	function initRefundForm(form) {
		form.addEventListener('submit', (e) => {
			...
			const statusEl   = form.querySelector('.delopay-refund-status');
