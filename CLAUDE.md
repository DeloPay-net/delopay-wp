...
podman-compose run --rm wpcli plugin activate delopay
...
bin/build-zips.sh                 # → dist/delopay.zip and dist/delopay-shop.zip
```
...
- `POST /wp-json/delopay/v1/webhook` verifies HMAC-SHA512 (header `x-webhook-signature-512`) **before** any state change; the secret comes from `Delopay_Settings`.
...
- Plugin-emitted classnames (`delopay-grid`, `delopay-product`, …) are safelisted in `tailwind.config.js` because they live outside the theme's content-scan paths.
...
- `phpstan-bootstrap.php` declares `DELOPAY_*` constants for static analysis only — it's not loaded at runtime.
